...
export default class AIPath {
    ...
    /**
     * Evento que se dispara cuando la ruta termina de generarse.
     */
    private onGenerated :GameEvent<() => void>;

    constructor(entity :Entity, destination :{x :number, y :number}) {
        this.entity = entity;
     ...
     */
    public async recalculate(newDest :{x :number, y :number}) {
     ...
     */
    public suscribe(instance :any, onGenerated :(() => void) | null) {
        if(onGenerated) {
     ...
     */
    public render(context :CanvasRenderingContext2D, scrollX :number, scrollY :number) {
        ...
        var collider = this.entity.getCollider() as Collider;
        if(!collider) {
            return;
        }
    ...
    private generatePoints() {
        ...
        var entityCollider = this.entity.getCollider() as Collider;
        ...
        if(colliders.nondynamicCollisionAtPosition(this.destination.x, this.destination.y)) {
            console.warn("Atención, Ruta Imposible: "
                + "Una entidad ha intentado generar una ruta hacia el tile sólido situado en (%d, %d)",
                destTile.x, destTile.y);
            // ¿Para qué perder el tiempo intentando crear la ruta si ya sabemos que es imposible?
            return;
        }

        // Los posibles tiles que vayamos a recorrer se guardan en este mapa, que le
        ...
        var astarmap = new Map<string, Tiledata>();
        ...
        while(tileQueue.length > 0) {
            ...
            let currentTileData = astarmap.get(this.toString(tileQueue[0])) as Tiledata;
        ...
        while(!this.tilesAreEqual(currentTile, startTile) || this.points.length == 0) {
            ...
            let currentTileData = astarmap.get(this.toString(currentTile)) as Tiledata;
     ...
     */
    private static getNeighbourTiles(tile :{x :number, y :number}) {
     ...
     */
    private areDiagonallyAdjacent(tile1 :{x: number, y :number}, tile2 :{x: number, y :number}) {
     ...
     */
    private getManhattanDistance(orig :{x :number, y :number}, dest :{x :number, y :number}) {
     ...
     */
    private toString(tile :{x :number, y :number}) {
